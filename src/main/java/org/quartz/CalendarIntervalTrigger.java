 ...
 * </code> based upon repeating calendar time intervals.
 *
 * <p>The trigger will fire every N (see {@link #getRepeatInterval()} ) units of calendar time (see
 * {@link #getRepeatIntervalUnit()}) as specified in the trigger's definition. This trigger can
 * achieve schedules that are not possible with {@link SimpleTrigger} (e.g because months are not a
 * fixed number of seconds) or {@link CronTrigger} (e.g. because "every 5 months" is not an even
 * divisor of 12).
 *
 * <p>If you use an interval unit of <code>MONTH</code> then care should be taken when setting a
 ...
 * the month - regardless of the number of days in the month, you should use <code>CronTrigger
 * </code>.
 *
 * @see TriggerBuilder
...
public interface CalendarIntervalTrigger extends Trigger {
   ...
   * {@link CalendarIntervalTrigger}</code> wants to be fired now by <code>Scheduler</code>.
   */
  int MISFIRE_INSTRUCTION_FIRE_ONCE_NOW = 1;

  /**
   * Instructs the <code>{@link Scheduler}</code> that upon a mis-fire situation, the <code>
   * {@link CalendarIntervalTrigger}</code> wants to have it's next-fire-time updated to the next
   * time in the schedule after the current time (taking into account any associated <code>
   * {@link Calendar}</code>, but it does not want to be fired now.
   */
   ...
   *
   * <p>If null, the system default TimeZone will be used.
   */
   ...
   *
   * <p>For example, without the property set, your trigger may have a start time of 9:00 am on
   * March 1st, and a repeat interval of 2 days. But after the daylight saving transition occurs,
   * the trigger may start firing at 8:00 am every other day.
   *
   * <p>If however, the time of day does not exist on a given day to fire (e.g. 2:00 am in the
   * United States on the days of daylight saving transition), the trigger will go ahead and fire
   * one hour off on that day, and then resume the normal hour on other days. If you wish for the
   * trigger to never fire at the "wrong" hour, then you should set the property
   ...
   *
   * <p><b>CAUTION!</b> If you enable this property, and your hour of day happens to be that of
