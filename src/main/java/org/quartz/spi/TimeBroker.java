 ...
 *
 * <p>The interface to be implemented by classes that want to provide a mechanism by which the
 * <code>{@link org.quartz.core.QuartzScheduler}</code> can reliably determine the current time.
 *
...
public interface TimeBroker {
   ...
   *
   * @throws SchedulerException with the error code set to
   *     SchedulerException.ERR_TIME_BROKER_FAILURE
   */
  Instant getCurrentTime() throws SchedulerException;

  /**
   * Called by the QuartzScheduler before the <code>TimeBroker</code> is used, in order to give the
   * it a chance to initialize.
   */
  void initialize() throws SchedulerConfigException;

  /**
   * Called by the QuartzScheduler to inform the <code>TimeBroker</code> that it should free up all
