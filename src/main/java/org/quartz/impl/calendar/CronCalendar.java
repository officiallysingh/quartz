...
/**
 * This implementation of the Calendar excludes the set of times expressed by a given {@link
 * org.quartz.CronExpression CronExpression}. For example, you could use this calendar to exclude
 * all but business hours (8AM - 5PM) every day using the expression &quot;* * 0-7,18-23 ? *
 * *&quot;.
 *
 * <p>It is important to remember that the cron expression here describes a set of times to be
 * <I>excluded</I> from firing. Whereas the cron expression in {@link org.quartz.CronTrigger
 * CronTrigger} describes a set of times that can be <I>included</I> for firing. Thus, if a <CODE>
 * CronTrigger</CODE> has a given cron expression and is associated with a <CODE>CronCalendar</CODE>
 * with the <I>same</I> expression, the calendar will exclude all the times the trigger includes,
...
public class CronCalendar extends BaseCalendar {
  ...
  /**
   * Create a <CODE>CronCalendar</CODE> with the given cron expression and no <CODE>baseCalendar
   * </CODE>.
   *
  ...
  /**
   * Create a <CODE>CronCalendar</CODE> with the given cron expression, <CODE>baseCalendar</CODE>,
   * and <code>TimeZone</code>.
   *
   * @param baseCalendar the base calendar for this calendar instance &ndash; see {@link
   *     BaseCalendar} for more information on base calendar functionality
   * @param expression a String representation of the desired cron expression
   * @param timeZone Specifies for which time zone the <code>expression</code> should be
   *     interpreted, i.e. the expression 0 0 10 * * ?, is resolved to 10:00 am in this time zone.
   *     If <code>timeZone</code> is <code>null</code> then <code>TimeZone.getDefault()</code> will
   *     be used.
   */
  public CronCalendar(Calendar baseCalendar, String expression, TimeZone timeZone)
  ...
  /**
   * Returns the time zone for which the <code>CronExpression</code> of this <code>CronCalendar
   * </code> will be resolved.
   *
   * <p>Overrides <code>{@link BaseCalendar#getTimeZone()}</code> to defer to its <code>
   * CronExpression</code>.
   */
  @Override
  ...
  /**
   * Sets the time zone for which the <code>CronExpression</code> of this <code>CronCalendar</code>
   * will be resolved. If <code>timeZone</code> is <code>null</code> then <code>
   * TimeZone.getDefault()</code> will be used.
   *
   * <p>Overrides <code>{@link BaseCalendar#setTimeZone(TimeZone)}</code> to defer to its <code>
   * CronExpression</code>.
   */
  @Override
   ...
   * @return a boolean indicating whether the specified time is 'included' by the <CODE>CronCalendar
   *     </CODE>
   */
  @Override
  ...
  /**
   * Returns the object representation of the cron expression that defines the dates and times this
   * calendar excludes.
   *
