 ...
 * day. Each <CODE>DailyCalendar</CODE> only allows a single time range to be specified, and that
 ...
 * defines a range of times in which triggers are not allowed to fire. If <CODE>invertTimeRange
 * </CODE> is <CODE>true</CODE>, the time range is inverted &ndash; that is, all times
 * <I>outside</I> the defined time range are excluded.
 *
 * <p>Note when using <CODE>DailyCalendar</CODE>, it behaves on the same principals as, for example,
 * {@link org.quartz.impl.calendar.WeeklyCalendar WeeklyCalendar}. <CODE>WeeklyCalendar</CODE>
 * defines a set of days that are excluded <I>every week</I>. Likewise, <CODE>DailyCalendar</CODE>
...
public class DailyCalendar extends BaseCalendar {
   ...
   * <CODE>baseCalendar</CODE>. <CODE>rangeStartingTime</CODE> and <CODE>rangeEndingTime</CODE> must
   * be in the format &quot;HH:MM[:SS[:mmm]]&quot; where:
   *
   * <UL>
   *   <LI>HH is the hour of the specified time. The hour should be specified using military
   *       (24-hour) time and must be in the range 0 to 23.
   *   <LI>MM is the minute of the specified time and must be in the range 0 to 59.
   *   <LI>SS is the second of the specified time and must be in the range 0 to 59.
   *   <LI>mmm is the millisecond of the specified time and must be in the range 0 to 999.
   *   <LI>items enclosed in brackets ('[', ']') are optional.
   *   <LI>The time range starting time must be before the time range ending time. Note this means
   *       that a time range may not cross daily boundaries (10PM - 2AM)
   * </UL>
   *
   * <p><b>Note:</b> This <CODE>DailyCalendar</CODE> will use the <code>
   * {@link TimeZone#getDefault()}</code> time zone unless an explicit time zone is set via <code>
   * {@link BaseCalendar#setTimeZone(TimeZone)}</code>
   *
  ...
  /**
   * Create a <CODE>DailyCalendar</CODE> with a time range defined by the specified strings and the
   * specified <CODE>baseCalendar</CODE>. <CODE>rangeStartingTime</CODE> and <CODE>rangeEndingTime
   * </CODE> must be in the format &quot;HH:MM[:SS[:mmm]]&quot; where:
   *
   * <UL>
   *   <LI>HH is the hour of the specified time. The hour should be specified using military
   *       (24-hour) time and must be in the range 0 to 23.
   ...
   * {@link BaseCalendar#setTimeZone(TimeZone)}</code>
   *
   * @param baseCalendar the base calendar for this calendar instance &ndash; see {@link
   *     BaseCalendar} for more information on base calendar functionality
   * @param rangeStartingTime a String representing the starting time for the time range
   * @param rangeEndingTime a String representing the ending time for the time range
   */
  public DailyCalendar(
      org.quartz.Calendar baseCalendar, String rangeStartingTime, String rangeEndingTime) {
    super(baseCalendar);
    setTimeRange(rangeStartingTime, rangeEndingTime);
  }
   ...
   *
   * <UL>
   *   <LI>Hours must be in the range 0-23 and are expressed using military (24-hour) time.
   *   <LI>Minutes must be in the range 0-59
   *   <LI>Seconds must be in the range 0-59
   *   <LI>Milliseconds must be in the range 0-999
   *   <LI>The time range starting time must be before the time range ending time. Note this means
   *       that a time range may not cross daily boundaries (10PM - 2AM)
   * </UL>
   *
  ...
  /**
   * Create a <CODE>DailyCalendar</CODE> with a time range defined by the specified values and the
   * specified <CODE>baseCalendar</CODE>. Values are subject to the following validations:
   *
   * <UL>
   *   <LI>Hours must be in the range 0-23 and are expressed using military (24-hour) time.
   *   <LI>Minutes must be in the range 0-59
  ...
  /**
   * Create a <CODE>DailyCalendar</CODE> with a time range defined by the specified <CODE>
   * java.util.Calendar</CODE>s and no <CODE>baseCalendar</CODE>. The Calendars are subject to the
   ...
   *       means that a time range may not cross daily boundaries (10PM - 2AM). <I>(because only
   *       time fields are are used, it is possible for two Calendars to represent a valid time
   *       range and <CODE>rangeStartingCalendar.after(rangeEndingCalendar) ==
   *         true</CODE>)</I>
   * </UL>
   *
   * <p><b>Note:</b> This <CODE>DailyCalendar</CODE> will use the <code>
   ...
   * {@link BaseCalendar#setTimeZone(TimeZone)}</code>
   *
   * @param rangeStartingCalendar a java.util.Calendar representing the starting time for the time
   *     range
   * @param rangeEndingCalendar a java.util.Calendar representing the ending time for the time range
   */
  ...
  /**
   * Create a <CODE>DailyCalendar</CODE> with a time range defined by the specified <CODE>
   * java.util.Calendar</CODE>s and the specified <CODE>baseCalendar</CODE>. The Calendars are
  ...
  /**
   * Create a <CODE>DailyCalendar</CODE> with a time range defined by the specified values and no
   * <CODE>baseCalendar</CODE>. The values are subject to the following considerations:
   *
   * <UL>
   *   <LI>Only the time-of-day portion of the specified values will be used
   *   <LI>The starting time must be before the ending time of the defined time range. Note this
   *       means that a time range may not cross daily boundaries (10PM - 2AM). <I>(because only
   *       time value are are used, it is possible for the two values to represent a valid time
   *       range and <CODE>rangeStartingTime &gt;
   *         rangeEndingTime</CODE>)</I>
   * </UL>
   *
   * <p><b>Note:</b> This <CODE>DailyCalendar</CODE> will use the <code>
   * {@link TimeZone#getDefault()}</code> time zone unless an explicit time zone is set via <code>
   * {@link BaseCalendar#setTimeZone(TimeZone)}</code>. You should use <code>
   * {@link #DailyCalendar(org.quartz.Calendar, java.util.TimeZone, long, long)}</code> if you don't
   * want the given <code>rangeStartingTimeInMillis</code> and <code>rangeEndingTimeInMillis</code>
   * to be evaluated in the default time zone.
   *
   * @param rangeStartingTimeInMillis a long representing the starting time for the time range
   * @param rangeEndingTimeInMillis a long representing the ending time for the time range
   */
  ...
  /**
   * Create a <CODE>DailyCalendar</CODE> with a time range defined by the specified values and the
   * specified <CODE>baseCalendar</CODE>. The values are subject to the following considerations:
   *
   * <UL>
   *   <LI>Only the time-of-day portion of the specified values will be used
   *   <LI>The starting time must be before the ending time of the defined time range. Note this
   ...
   * {@link TimeZone#getDefault()}</code> time zone unless an explicit time zone is set via <code>
   * {@link BaseCalendar#setTimeZone(TimeZone)}</code>. You should use <code>
   * {@link #DailyCalendar(org.quartz.Calendar, java.util.TimeZone, long, long)} </code> if you
   * don't want the given <code>rangeStartingTimeInMillis</code> and <code>rangeEndingTimeInMillis
   * </code> to be evaluated in the default time zone.
   *
   * @param baseCalendar the base calendar for this calendar instance &ndash; see {@link
   *     BaseCalendar} for more information on base calendar functionality
   * @param rangeStartingTimeInMillis a long representing the starting time for the time range
   ...
   * </UL>
   *
   * @param timeZone the time zone for of the <code>DailyCalendar</code> which will also be used to
   *     resolve the given start/end times.
   * @param rangeStartingTimeInMillis a long representing the starting time for the time range
   * @param rangeEndingTimeInMillis a long representing the ending time for the time range
   */
  public DailyCalendar(
      TimeZone timeZone, long rangeStartingTimeInMillis, long rangeEndingTimeInMillis) {
    super(timeZone);
    setTimeRange(rangeStartingTimeInMillis, rangeEndingTimeInMillis);
  }
   ...
   * @return a boolean indicating whether the specified time is 'included' by the <CODE>BaseCalendar
   *     </CODE>
   */
  @Override
  ...
  /**
   * Returns the start time of the time range (in milliseconds) of the day specified in <CODE>
   * timeInMillis</CODE>
   *
  ...
  /**
   * Returns the end time of the time range (in milliseconds) of the day specified in <CODE>
   * timeInMillis</CODE>
   *
  ...
  /**
   * Sets the time range for the <CODE>DailyCalendar</CODE> to the times represented in the
   * specified Strings.
   *
   ...
   *
   * @param rangeStartingCalendar a Calendar containing the start time for the <CODE>DailyCalendar
   *     </CODE>
   * @param rangeEndingCalendar a Calendar containing the end time for the <CODE>DailyCalendar
   *     </CODE>
   */
   ...
   * specified values.
   *
   * @param rangeStartingTime the starting time (in milliseconds) for the time range
   * @param rangeEndingTime the ending time (in milliseconds) for the time range
   */
