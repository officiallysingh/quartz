 ...
 *
 * <p>This builder provide an extra convenient method for you to set the trigger's endTimeOfDay. You
 ...
 * calculation.
 *
 * <p>When using endingDailyAfterCount(), you should note that it is used to calculating
 * endTimeOfDay. So if your startTime on the first day is already pass by a time that would not add
 * up to the count you expected, until the next day comes. Remember that DailyTimeIntervalTrigger
 * will use startTimeOfDay and endTimeOfDay as fresh per each day!
 *
...
public class DailyTimeIntervalScheduleBuilder extends ScheduleBuilder<DailyTimeIntervalTrigger> {
   ...
   *
   * <p>The set contains all values between {@link java.util.Calendar#SUNDAY} and {@link
   * java.util.Calendar#SATURDAY} (the integers from 1 through 7).
   */
   ...
   *
   * <p>The set contains all values between {@link java.util.Calendar#MONDAY} and {@link
   * java.util.Calendar#FRIDAY} (the integers from 2 through 6).
   */
   ...
   *
   * <p>The set contains {@link java.util.Calendar#SATURDAY} and {@link java.util.Calendar#SUNDAY}
   */
  public static final Set<Integer> SATURDAY_AND_SUNDAY;

  static {
    Set<Integer> t = new HashSet<>(7);
    for (int i = Calendar.SUNDAY; i <= Calendar.SATURDAY; i++) t.add(i);
    ALL_DAYS_OF_THE_WEEK = Collections.unmodifiableSet(t);

    t = new HashSet<>(5);
    for (int i = Calendar.MONDAY; i <= Calendar.FRIDAY; i++) t.add(i);
  ...
  public MutableTrigger build() {
    ...
    if (daysOfWeek != null) st.setDaysOfWeek(daysOfWeek);
    else st.setDaysOfWeek(ALL_DAYS_OF_THE_WEEK);

    if (startTimeOfDay != null) st.setStartTimeOfDay(startTimeOfDay);
    else st.setStartTimeOfDay(TimeOfDay.hourAndMinuteOfDay(0, 0));

    if (endTimeOfDay != null) st.setEndTimeOfDay(endTimeOfDay);
    else st.setEndTimeOfDay(TimeOfDay.hourMinuteAndSecondOfDay(23, 59, 59));
   ...
   * @param unit the time unit (IntervalUnit) of the interval. The only intervals that are valid for
   *     this type of trigger are {@link IntervalUnit#SECOND}, {@link IntervalUnit#MINUTE}, and
  ...
  public DailyTimeIntervalScheduleBuilder withInterval(int timeInterval, IntervalUnit unit) {
    if (unit == null
        || !(unit.equals(IntervalUnit.SECOND)
            || unit.equals(IntervalUnit.MINUTE)
            || unit.equals(IntervalUnit.HOUR)))
   ...
   *
   * @param onDaysOfWeek a Set containing the integers representing the days of the week, per the
   *     values 1-7 as defined by {@link java.util.Calendar#SUNDAY} - {@link
   ...
   * Set the trigger to fire on the given days of the week.
   *
   * @param onDaysOfWeek a variable length list of Integers representing the days of the week, per
   *     the values 1-7 as defined by {@link java.util.Calendar#SUNDAY} - {@link
   *     java.util.Calendar#SATURDAY}.
   * @return the updated DailyTimeIntervalScheduleBuilder
   */
  ...
  public DailyTimeIntervalScheduleBuilder endingDailyAfterCount(int count) {
    ...
    else if (intervalUnit == IntervalUnit.HOUR) intervalInMillis = interval * 1000L * 60 * 60;
    else
      throw new IllegalArgumentException(
          "The IntervalUnit: " + intervalUnit + " is invalid for this trigger.");

    if (remainingMillisInDay - intervalInMillis <= 0)
      throw new IllegalArgumentException(
          "The startTimeOfDay is too late with given Interval and IntervalUnit values.");
   ...
   *
   * <p>Note: if you want total count = 1 (at start time) + repeatCount
   *
   * @return the new DailyTimeIntervalScheduleBuilder
   */
