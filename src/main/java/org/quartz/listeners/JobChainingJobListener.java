 ...
 *
 * <p>Generally an instance of this listener would be registered as a global job listener, rather
 * than being registered directly to a given job.
 *
 * <p>If for some reason there is a failure creating the trigger for the follow-up job (which would
 * generally only be caused by a rare serious failure in the system, or the non-existence of the
 * follow-up job), an error message is logged, but no other action is taken. If you need more
