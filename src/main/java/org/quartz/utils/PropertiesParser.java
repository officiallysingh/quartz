...
public class PropertiesParser {
   ...
   *
   * @param prefix The prefix for which to search. If it does not end in a "." then one will be
   *     added to it for search purposes.
   * @param stripPrefix Whether to strip off the given <code>prefix</code> in the result's keys.
   * @param excludedPrefixes Optional array of fully qualified prefixes to exclude. For example if
   *     <code>prefix</code> is "a.b.c", then <code>excludedPrefixes</code> might be "a.b.c.ignore".
   * @return Group of <code>Properties</code> that start with the given prefix, optionally have that
   *     prefix removed, and do not include properties that start with one of the given excluded
   *     prefixes.
   */
  public Properties getPropertyGroup(
      String prefix, boolean stripPrefix, String[] excludedPrefixes) {
    Enumeration<?> keys = props.propertyNames();
