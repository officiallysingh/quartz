 ...
 *
 * <ul>
 *   <li><code>Job execution</code>
 *   <li><code>{@link JobListener} methods</code>
 *   <li><code>{@link TriggerListener} methods</code>
 * </ul>
 *
 * The exception ensures that the job execution context is transferred to the implementation of the
 * error handling method of the scheduler listener <code>
 * {@link org.quartz.SchedulerListener#schedulerError(String, SchedulerException)}</code>, to try to
 * fix the problem
 */
public class JobExecutionProcessException extends SchedulerException {
  ...
  public JobExecutionProcessException(
      JobListener listener, JobExecutionContext jobExecutionContext, Throwable cause) {
    super(
        String.format(JobListenerExecution.getErrorMsg(), listener.getName(), cause.getMessage()),
        cause);
    this.jobExecutionContext = jobExecutionContext;
  }

  public JobExecutionProcessException(
      TriggerListener listener, JobExecutionContext jobExecutionContext, Throwable cause) {
    super(
        String.format(
            TriggerListenerExecution.getErrorMsg(), listener.getName(), cause.getMessage()),
        cause);
    this.jobExecutionContext = jobExecutionContext;
  }
