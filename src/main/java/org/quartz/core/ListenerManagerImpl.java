...
public class ListenerManagerImpl implements ListenerManager {
  ...
  private final Map<String, List<Matcher<JobKey>>> globalJobListenersMatchers =
      new LinkedHashMap<>(10);

  private final Map<String, List<Matcher<TriggerKey>>> globalTriggerListenersMatchers =
      new LinkedHashMap<>(10);
  ...
  public void addJobListener(JobListener jobListener, List<Matcher<JobKey>> matchers) {
    ...
    synchronized (globalJobListeners) {
      ...
      if (matchers != null && !matchers.isEmpty()) matchersL.addAll(matchers);
      else matchersL.add(EverythingMatcher.allJobs());
  ...
  public void addJobListener(JobListener jobListener, Matcher<JobKey> matcher) {
    ...
    synchronized (globalJobListeners) {
      globalJobListeners.put(jobListener.getName(), jobListener);
      LinkedList<Matcher<JobKey>> matchersL = new LinkedList<>();
      if (matcher != null) matchersL.add(matcher);
      else matchersL.add(EverythingMatcher.allJobs());
  ...
  public boolean addJobListenerMatcher(String listenerName, Matcher<JobKey> matcher) {
    if (matcher == null) throw new IllegalArgumentException("Null value not acceptable.");
  ...
  public boolean removeJobListenerMatcher(String listenerName, Matcher<JobKey> matcher) {
    if (matcher == null) throw new IllegalArgumentException("Non-null value not acceptable.");
  ...
  public boolean setJobListenerMatchers(String listenerName, List<Matcher<JobKey>> matchers) {
    if (matchers == null) throw new IllegalArgumentException("Non-null value not acceptable.");

    synchronized (globalJobListeners) {
      List<Matcher<JobKey>> oldMatchers = globalJobListenersMatchers.get(listenerName);
      if (oldMatchers == null) return false;
  ...
  public void addTriggerListener(
      TriggerListener triggerListener, List<Matcher<TriggerKey>> matchers) {
    ...
    synchronized (globalTriggerListeners) {
      ...
      if (matchers != null && !matchers.isEmpty()) matchersL.addAll(matchers);
      else matchersL.add(EverythingMatcher.allTriggers());
  ...
  public boolean setTriggerListenerMatchers(
      String listenerName, List<Matcher<TriggerKey>> matchers) {
    if (matchers == null) throw new IllegalArgumentException("Non-null value not acceptable.");
  ...
  public List<TriggerListener> getTriggerListeners() {
    synchronized (globalTriggerListeners) {
      return java.util.Collections.unmodifiableList(
          new LinkedList<>(globalTriggerListeners.values()));
    }
  }
