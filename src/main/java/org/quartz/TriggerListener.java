...
public interface TriggerListener {
   ...
   * it's associated <code>{@link org.quartz.JobDetail}</code> is about to be executed.
   *
  ...
  /**
   * Called by the <code>{@link Scheduler}</code> when a <code>{@link Trigger}</code> has fired, and
   * it's associated <code>{@link org.quartz.JobDetail}</code> is about to be executed. If the
   * implementation vetoes the execution (via returning <code>true</code>), the job's execute method
   * will not be called.
   *
   ...
   * it's associated <code>{@link org.quartz.JobDetail}</code> has been executed, and it's <code>
   * triggered(xx)</code> method has been called.
   *
