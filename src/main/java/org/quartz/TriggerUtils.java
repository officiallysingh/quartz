...
public class TriggerUtils {
   ...
   */
  public static List<Instant> computeFireTimes(
      OperableTrigger trigger, org.quartz.Calendar cal, int numTimes) {
  ...
  /**
   * Compute the <code>Instant</code> that is 1 second after the Nth firing of the given <code>
   * Trigger</code>, taking the trigger's associated <code>Calendar</code> into consideration.
   *
   * <p>The input trigger will be cloned before any work is done, so you need not worry about its
   * state being altered by this method.
   *
   * @param trigger The trigger upon which to do the work
   * @param cal The calendar to apply to the trigger's schedule
   * @param numTimes The number of next fire times to produce
   * @return the computed Instant, or null if the trigger (as configured) will not fire that many
   *     times.
   */
  public static Instant computeEndTimeToAllowParticularNumberOfFirings(
      OperableTrigger trigger, org.quartz.Calendar cal, int numTimes) {
   ...
   * firings which have already occurred will not be listed in the output List.
   *
   * @param trigger The trigger upon which to do the work
   * @param cal The calendar to apply to the trigger's schedule
  ...
  public static List<Instant> computeFireTimesBetween(
      OperableTrigger trigger, org.quartz.Calendar cal, Instant from, Instant to) {
    LinkedList<Instant> lst = new LinkedList<>();
