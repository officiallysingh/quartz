 ...
 *
 * <p>A <code>Scheduler</code> maintains a registry of <code>{@link org.quartz.JobDetail}</code>s
 * and <code>{@link Trigger}</code>s. Once registered, the <code>Scheduler</code> is responsible for
 * executing <code>Job</code> s when their associated <code>Trigger</code> s fire (when their
 * scheduled time arrives).
 *
 * <p><code>Scheduler</code> instances are produced by a <code>{@link SchedulerFactory}</code>. A
 ...
 * mode, and must have its <code>start()</code> method called before it will fire any <code>Job
 * </code>s.
 *
 * <p><code>Job</code> s are to be created by the 'client program', by defining a class that
 * implements the <code>{@link org.quartz.Job}</code> interface. <code>{@link JobDetail}</code>
 * objects are then created (also by the client) to define a individual instances of the <code>Job
 * </code>. <code>JobDetail</code> instances can then be registered with the <code>Scheduler</code>
 * via the <code>scheduleJob(JobDetail, Trigger)</code> or <code>addJob(JobDetail, boolean)</code>
 * method.
 *
 * <p><code>Trigger</code> s can then be defined to fire individual <code>Job</code> instances based
 * on given schedules. <code>SimpleTrigger</code> s are most useful for one-time firings, or firing
 * at an exact moment in time, with N repeats with a given delay between them. <code>CronTrigger
 * </code> s allow scheduling based on time of day, day of week, day of month, and month of year.
 *
 * <p><code>Job</code> s and <code>Trigger</code> s have a name and group associated with them,
 * which should uniquely identify them within a single <code>{@link Scheduler}</code>. The 'group'
 * feature may be useful for creating logical groupings or categorizations of <code>Jobs</code> s
 * and <code>Triggers</code>s. If you don't have need for assigning a group to a given <code>Jobs
 * </code> of <code>Triggers</code>, then you can use the <code>DEFAULT_GROUP</code> constant
 * defined on this interface.
 *
 * <p>Stored <code>Job</code> s can also be 'manually' triggered through the use of the <code>
 * triggerJob(String jobName, String jobGroup)</code> function.
 *
 * <p>Client programs may also be interested in the 'listener' interfaces that are available from
 * Quartz. The <code>{@link JobListener}</code> interface provides notifications of <code>Job</code>
 * executions. The <code>{@link TriggerListener}</code> interface provides notifications of <code>
 * Trigger</code> firings. The <code>{@link SchedulerListener}</code> interface provides
 * notifications of <code>Scheduler</code> events and errors. Listeners can be associated with local
 * schedulers through the {@link ListenerManager} interface.
 *
 * <p>The setup/configuration of a <code>Scheduler</code> instance is very customizable. Please
...
public interface Scheduler {
  ...
  /**
   * A constant <code>Trigger</code> group name used internally by the scheduler - clients should
   * not use the value of this constant ("FAILED_OVER_JOBS") for the name of a <code>Trigger</code>
   * 's group.
   *
   * @see org.quartz.JobDetail#requestsRecovery()
   */
   ...
   *
   * <p>Note that this is the time the original firing actually occurred, which may be different
   * from the scheduled fire time - as a trigger doesn't always fire exactly on time.
   *
   * @see org.quartz.JobDetail#requestsRecovery()
   */
   ...
   * recovering after a failed scheduler instance.
   *
   * <p>Note that this is the time the original firing was scheduled for, which may be different
   * from the actual firing time - as a trigger doesn't always fire exactly on time.
   *
   * @see org.quartz.JobDetail#requestsRecovery()
   */
  ...
  /**
   * Starts the <code>Scheduler</code>'s threads that fire <code>{@link Trigger}s</code>. When a
   ...
   *
   * <p>The misfire/recovery process will be started, if it is the initial call to this method on
   ...
   *
   * <p>When <code>start()</code> is called (to bring the scheduler out of stand-by mode), trigger
  ...
  /**
   * Halts the <code>Scheduler</code>'s firing of <code>{@link Trigger}s</code>, and cleans up all
   ...
   * scheduler instance.
   *
   * <p>Note that the data returned is an 'instantaneous' snap-shot, and that as soon as it's
   * returned, the meta data values may be different.
   */
   ...
   * returned, the true list of executing jobs may be different. Also please read the doc associated
   * with <code>JobExecutionContext</code>- especially if you're using RMI.
   *
  ...
  /**
   * Set the <code>JobFactory</code> that will be responsible for producing instances of <code>Job
   * </code> classes.
   *
  ...
  /**
   * Add the given <code>{@link org.quartz.JobDetail}</code> to the Scheduler, and associate the
   * given <code>{@link Trigger}</code> with it.
   *
   ...
   */
  Instant scheduleJob(JobDetail jobDetail, Trigger trigger) throws SchedulerException;
   ...
   *
   * <p>If any of the given jobs or triggers already exist (or more specifically, if the keys are
   * not unique) and the replace parameter is not set to true then an exception will be thrown.
   *
   * @throws ObjectAlreadyExistsException if the job/trigger keys are not unique and the replace
   ...
   *
   * <p>If any of the given job or triggers already exist (or more specifically, if the keys are not
   ...
   *
   * <p>If the related job does not have any other triggers, and the job is not durable, then the
   ...
   * job will also be deleted.
   *
   * <p>Note that while this bulk operation is likely more efficient than invoking <code>
   * unscheduleJob(TriggerKey triggerKey)</code> several times, it may have the adverse affect of
  ...
  /**
   * Remove (delete) the <code>{@link org.quartz.Trigger}</code> with the given key, and store the
   * new given one - which must be associated with the same job (the new trigger must have the job
   * name and group specified) - however, the new trigger need not have the same name as the old
   * trigger.
   *
   ...
   *     and removed from the store (and the new trigger is therefore not stored), otherwise the
   *     first fire time of the newly scheduled trigger is returned.
   */
  Instant rescheduleJob(TriggerKey triggerKey, Trigger newTrigger) throws SchedulerException;
   ...
   * Scheduler.triggerJob()</code> is called for it.
   *
   * <p>With the <code>storeNonDurableWhileAwaitingScheduling</code> parameter set to <code>true
   * </code>, a non-durable job can be stored. Once it is scheduled, it will resume normal
  ...
  /**
   * Delete the identified <code>Job</code>s from the Scheduler - and any associated <code>Trigger
   * </code>s.
   *
   * <p>Note that while this bulk operation is likely more efficient than invoking <code>
   * deleteJob(JobKey jobKey)</code> several times, it may have the adverse affect of holding data
   * locks for a single long duration of time (rather than lots of small durations of time).
   *
   ...
   * Trigger the identified <code>{@link org.quartz.JobDetail}</code> (execute it now).
   *
   * @param data the (possibly <code>null</code>) JobDataMap to be associated with the trigger that
   *     fires the job immediately.
   */
  void triggerJob(JobKey jobKey, JobDataMap data) throws SchedulerException;

  /**
   * Pause the <code>{@link org.quartz.JobDetail}</code> with the given key - by pausing all of its
   ...
   *
   * <p>The Scheduler will "remember" the groups paused, and impose the pause on any new jobs that
   * are added to any of those groups until it is resumed.
   *
   * <p>NOTE: There is a limitation that only exactly matched groups can be remembered as paused.
   * For example, if there are preexisting job in groups "aaa" and "bbb" and a matcher is given to
   ...
   * applied. HOWEVER, if there are preexisting groups "aaa" and "bbb" and a matcher is given to
   ...
   *
   * <p>The Scheduler will "remember" all the groups paused, and impose the pause on any new
   * triggers that are added to any of those groups until it is resumed.
   *
   * <p>NOTE: There is a limitation that only exactly matched groups can be remembered as paused.
   * For example, if there are preexisting triggers in groups "aaa" and "bbb" and a matcher is given
   * to pause groups that start with "a" then the group "aaa" will be remembered as paused and any
   ...
   * matcher was applied. HOWEVER, if there are preexisting groups "aaa" and "bbb" and a matcher is
  ...
  /**
   * Gets all the {@link org.quartz.JobDetail Jobdetails} in the matching groups.
   *
   * <p>The returned JobDetail objects will be a snap-shot of the actual stored JobDetail. If you
   * wish to modify the JobDetail, you must re-store the JobDetail afterward (e.g. see {@link
   * #addJob(JobDetail, boolean)}).
   *
   * @param matcher Matcher to evaluate against known groups
   ...
   * @throws SchedulerException On error
   */
  List<JobDetail> getJobDetails(GroupMatcher<JobKey> matcher) throws SchedulerException;
   ...
   *
   * @param updateTriggers whether or not to update existing triggers that referenced the already
   ...
   *
   * <p>If removal of the <code>Calendar</code> would result in <code>Trigger</code>s pointing to
   * nonexistent calendars, then a <code>SchedulerException</code> will be thrown.
   *
   * @return true if the Calendar was found and deleted.
   * @throws SchedulerException if there is an internal Scheduler error, or one or more triggers
   ...
   * of the identified <code>Job</code>, which must be an implementor of the <code>InterruptableJob
   * </code> interface.
   *
   * <p>If more than one instance of the identified job is currently executing, the <code>
   * InterruptableJob#interrupt()</code> method will be called on each instance. However, there is a
   * limitation that in the case that <code>interrupt()</code> on one instances throws an exception,
   * all remaining instances (that have not yet been interrupted) will not have their <code>
   * interrupt()</code> method called.
   *
   ...
   * @throws UnableToInterruptJobException if the job does not implement <code>InterruptableJob
   *     </code>, or there is an exception while interrupting the job.
  ...
  /**
   * Request the interruption, within this Scheduler instance, of the identified executing <code>Job
   * </code> instance, which must be an implementor of the <code>InterruptableJob</code> interface.
   *
   * <p>This method is not cluster aware. That is, it will only interrupt instances of the
   * identified InterruptableJob currently executing in this Scheduler instance, not across the
   * entire cluster.
