...
public class JobBuilder {
  ...
  public JobDetail build() {
    ...
    if (key == null) key = new JobKey(Key.createUniqueName(null), null);
    job.setKey(key);
    job.setDurability(durability);
    job.setRequestsRecovery(shouldRecover);

    if (!jobDataMap.isEmpty()) job.setJobDataMap(jobDataMap);
   ...
   *
   * <p>If not explicitly set, the default value is <code>false</code>. - this method sets the value
   * to <code>true</code>.
   *
   * @return the updated JobBuilder
  ...
  /**
   * Whether or not the <code>Job</code> should remain stored after it is orphaned (no <code>
  ...
  /**
   * Add all the data from the given {@link JobDataMap} to the {@code JobDetail}'s {@code
   * JobDataMap}.
   *
   * @return the updated JobBuilder
   * @see JobDetail#getJobDataMap()
