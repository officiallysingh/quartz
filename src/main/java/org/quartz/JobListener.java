 ...
 * {@link org.quartz.JobDetail}</code> executes. In general, applications that use a <code>Scheduler
 * </code> will not have use for this mechanism.
 *
...
public interface JobListener {
   ...
   * is about to be executed (an associated <code>{@link Trigger}</code> has occurred).
   *
  ...
  /**
   * Called by the <code>{@link Scheduler}</code> when a <code>{@link org.quartz.JobDetail}</code>
   * was about to be executed (an associated <code>{@link Trigger}</code> has occurred), but a
   ...
   * has been executed, and be for the associated <code>Trigger</code>'s <code>triggered(xx)</code>
