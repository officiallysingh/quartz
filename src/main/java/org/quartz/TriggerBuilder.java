 ...
 *
 * <p>The builder will always try to keep itself in a valid state, with reasonable defaults set for
...
public class TriggerBuilder<T extends Trigger> {
  ...
  public T build() {
    ...
    if (scheduleBuilder == null) scheduleBuilder = SimpleScheduleBuilder.simpleSchedule();
    ...
    if (key == null) key = new TriggerKey(Key.createUniqueName(null), null);
    trig.setKey(key);
    if (jobKey != null) trig.setJobKey(jobKey);
    trig.setPriority(priority);

    if (!jobDataMap.isEmpty()) trig.setJobDataMap(jobDataMap);
  ...
  /**
   * the trigger may or may not fire at this time - depending upon the schedule configured for the
   * Trigger.
   *
   * @return the updated TriggerBuilder
   * @see Trigger#getStartTime()
  ...
  /**
   * Set the identity of the Job which should be fired by the produced Trigger - a <code>JobKey
   * </code> will be produced with the given name and default group.
   *
  ...
  /**
   * Set the identity of the Job which should be fired by the produced Trigger - a <code>JobKey
   * </code> will be produced with the given name and group.
   *
