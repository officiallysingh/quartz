 ...
 *
 * <p>If startTime is before startTimeOfDay, then it has no affect. Else if startTime after
...
public interface DailyTimeIntervalTrigger extends Trigger {
   ...
   * {@link DailyTimeIntervalTrigger}</code> wants to be fired now by <code>Scheduler</code>.
   */
  int MISFIRE_INSTRUCTION_FIRE_ONCE_NOW = 1;

  /**
   * Instructs the <code>{@link Scheduler}</code> that upon a mis-fire situation, the <code>
   * {@link DailyTimeIntervalTrigger}</code> wants to have it's next-fire-time updated to the next
  ...
  /**
   * Get the number of times for interval this trigger should repeat, after which it will be
   ...
   *
   * @return a Set containing the integers representing the days of the week, per the values 1-7 as
