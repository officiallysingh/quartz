 ...
 * was inspected. If the file has been updated, the job invokes a "call-back" method on an
 * identified <code>FileScanListener</code> that can be found in the <code>SchedulerContext</code>.
 *
 * @author jhouse
 * @author pl47ypus
 * @see FileScanListener
 */
...
public class FileScanJob implements Job {
  ...
  /**
   * <code>JobDataMap</code> key with which to specify the {@link FileScanListener} to be notified
   * when the file contents change.
   */
  public static final String FILE_SCAN_LISTENER_NAME = "FILE_SCAN_LISTENER_NAME";

  /**
   * <code>JobDataMap</code> key with which to specify a <code>long</code> value that represents the
   * minimum number of milliseconds that must have past since the file's last modified time in order
   * to consider the file new/altered. This is necessary because another process may still be in the
   * middle of writing to the file when the scan occurs, and the file may therefore not yet be ready
   * for processing.
   *
   * <p>If this parameter is not specified, a default value of <code>5000</code> (five seconds) will
   * be used.
   */
