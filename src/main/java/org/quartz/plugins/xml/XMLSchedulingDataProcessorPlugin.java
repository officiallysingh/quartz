 ...
 *
 * <p>The periodically scanning of files for changes is not currently supported in a clustered
 * environment.
 *
...
public class XMLSchedulingDataProcessorPlugin implements SchedulerPlugin, FileScanListener {
  ...
  public void initialize(
      String name, final Scheduler scheduler, ClassLoadHelper schedulerFactoryClassLoadHelper)
          ...
          if (scanInterval > 0) {
            ...
            JobDetail job =
                newJob()
                    ...
                    .ofType(FileScanJob.class)
                    .usingJobData(FileScanJob.FILE_NAME, jobFile.getFileName())
                    .usingJobData(
                        FileScanJob.FILE_SCAN_LISTENER_NAME,
                        JOB_INITIALIZATION_PLUGIN_NAME + '_' + getName())
                    .build();
            ...
            getLog()
                .debug(
                    "Scheduled file scan job for data file: {}, at interval: {}",
                    jobFile.getFileName(),
                    scanInterval);
          }
  ...
  /**
   * Helper method for generating unique job/trigger name for the file scanning jobs (one per
   * FileJob). The unique names are saved in jobTriggerNameSet.
   */
  private String buildJobTriggerName(String fileBasename) {
    ...
    String jobTriggerName =
        JOB_INITIALIZATION_PLUGIN_NAME + '_' + getName() + '_' + fileBasename.replace('.', '_');
    ...
    while (!jobTriggerNameSet.add(jobTriggerName)) {
      ...
      if (jobTriggerName.length() > (MAX_JOB_TRIGGER_NAME_LEN - numericSuffix.length())) {
        jobTriggerName =
            jobTriggerName.substring(0, (MAX_JOB_TRIGGER_NAME_LEN - numericSuffix.length()));
      }
