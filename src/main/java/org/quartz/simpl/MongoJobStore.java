 ...
 * <p>
 * Inject an existing {@link MongoClient} (Spring Boot) via {@link #setMongoClient(MongoClient)}
 * and this store will not close it on shutdown. Otherwise it creates a client from {@code mongoUri}.
 */
public class MongoJobStore extends RAMJobStore {
    ...
    public void initialize(ClassLoadHelper loadHelper, SchedulerSignaler schedSignaler) {
        ...
        try {
            ...
            jobs.createIndex(new Document("schedName", 1).append("name", 1).append("group", 1), new IndexOptions().unique(true));
            triggers.createIndex(new Document("schedName", 1).append("name", 1).append("group", 1), new IndexOptions().unique(true));
            triggers.createIndex(new Document("schedName", 1).append("state", 1).append("nextFireTime", 1));
            locks.createIndex(new Document("schedName", 1).append("lockName", 1), new IndexOptions().unique(true));
            reloadFromMongo();
    ...
    public void shutdown() {
        ...
        try {
            locks.deleteMany(Filters.and(Filters.eq("schedName", instanceName), Filters.eq("owner", instanceId)));
            schedulerState.deleteOne(Filters.and(Filters.eq("schedName", instanceName), Filters.eq("instanceId", instanceId)));
    ...
    private boolean obtainLock() {
        ...
        Document result = locks.findOneAndUpdate(
                Filters.and(
                        Filters.eq("schedName", instanceName),
                        Filters.eq("lockName", "TRIGGER_ACCESS"),
                        Filters.or(Filters.eq("owner", instanceId), Filters.lte("expires", Instant.now()), Filters.exists("owner", false))),
                Updates.combine(
                        Updates.set("owner", instanceId),
                        Updates.set("expires", expiry)),
                new com.mongodb.client.model.FindOneAndUpdateOptions().upsert(true));
        if (result == null) {
            try {
                locks.insertOne(new Document("schedName", instanceName)
                ...
                while (System.currentTimeMillis() < waitUntil) {
                    Document existing = locks.find(Filters.and(
                            Filters.eq("schedName", instanceName),
                            Filters.eq("lockName", "TRIGGER_ACCESS"),
                            Filters.or(Filters.eq("owner", instanceId), Filters.lte("expires", Instant.now())))).first();
                    if (existing != null) {
                        locks.updateOne(Filters.eq("_id", existing.getObjectId("_id")),
    ...
    private void releaseLock() {
        locks.updateOne(
                Filters.and(Filters.eq("schedName", instanceName), Filters.eq("lockName", "TRIGGER_ACCESS"), Filters.eq("owner", instanceId)),
                Updates.unset("owner"));
    }
    ...
    private void persistAll() {
        ...
        for (JobWrapper jw : jobsByKey.values()) {
            ...
            jobs.replaceOne(Filters.and(Filters.eq("schedName", instanceName), Filters.eq("name", jw.key.getName()), Filters.eq("group", jw.key.getGroup())), doc, upsert);
        }
        for (TriggerWrapper tw : triggersByKey.values()) {
            ...
            triggers.replaceOne(Filters.and(Filters.eq("schedName", instanceName), Filters.eq("name", tw.key.getName()), Filters.eq("group", tw.key.getGroup())), doc, upsert);
        }
        for (Map.Entry<String, Calendar> e : calendarsByName.entrySet()) {
            ...
            calendars.replaceOne(Filters.and(Filters.eq("schedName", instanceName), Filters.eq("name", e.getKey())), doc, upsert);
        }
    ...
    private static <T> T deserialize(Binary binary) {
        ...
        try (ObjectInputStream ois = new ObjectInputStream(new ByteArrayInputStream(binary.getData()))) {
