...
/**
 * In-memory {@link JobStore}. Persistent storage is {@link org.quartz.impl.mongodb.MongoJobStore},
 * a JDBC-style document store that does not use this class.
 *
...
public class RAMJobStore implements JobStore {
  ...
  protected final HashMap<String, HashMap<TriggerKey, TriggerWrapper>> triggersByGroup =
      new HashMap<>(25);

  protected final TreeSet<TriggerWrapper> timeTriggers =
      new TreeSet<>(new TriggerWrapperComparator());
    ...
    synchronized (lock) {
      ...
      if (!repl) {
        ...
        HashMap<JobKey, JobWrapper> grpMap =
            jobsByGroup.computeIfAbsent(newJob.getKey().getGroup(), k -> new HashMap<>(100));
  ...
  /**
   * Remove (delete) the <code>{@link org.quartz.Job}</code> with the given name, and any <code>
  ...
  public boolean removeJobs(List<JobKey> jobKeys) throws JobPersistenceException {
    ...
    synchronized (lock) {
      for (JobKey key : jobKeys) allFound = removeJob(key) && allFound;
    }
  ...
  public boolean removeTriggers(List<TriggerKey> triggerKeys) throws JobPersistenceException {
    ...
    synchronized (lock) {
      for (TriggerKey key : triggerKeys) allFound = removeTrigger(key) && allFound;
    }
    ...
    synchronized (lock) {
      ...
      if (!replace) {
        for (Entry<JobDetail, Set<? extends Trigger>> e : triggersAndJobs.entrySet()) {
          if (checkExists(e.getKey().getKey())) throw new ObjectAlreadyExistsException(e.getKey());
  ...
  public void storeTrigger(OperableTrigger newTrigger, boolean replaceExisting)
    ...
    synchronized (lock) {
      ...
      List<TriggerWrapper> jobList =
          triggersByJob.computeIfAbsent(tw.jobKey, k -> new ArrayList<>(1));
      jobList.add(tw);

      // add to triggers by group
      HashMap<TriggerKey, TriggerWrapper> grpMap =
          triggersByGroup.computeIfAbsent(newTrigger.getKey().getGroup(), k -> new HashMap<>(100));
  ...
  /**
   * Retrieve all the matched <code>{@link org.quartz.JobDetail}</code> for the given <code>
   * {@link org.quartz.impl.matchers.GroupMatcher}</code>.
   *
      ...
      for (JobKey jobKey : matchingJobKeys) {
        ...
        for (OperableTrigger trigger : jobTriggers) {
          ...
          if (triggerMatcher
              ...
              .evaluate(trigger.getKey().getGroup(), triggerMatcher.getCompareToValue())) {
  ...
  /**
   * Get the number of <code>{@link org.quartz.JobDetail}</code> s that are stored in the <code>
   * JobsStore</code>.
   */
  ...
  /**
   * Get the names of all of the <code>{@link org.quartz.Job}</code> s that match the given
   * groupMatcher.
   */
  ...
  /**
   * Get the names of all of the <code>{@link org.quartz.Trigger}</code> s that match the given
   * groupMatcher.
   */
  public Set<TriggerKey> getTriggerKeys(GroupMatcher<TriggerKey> matcher) {
    ...
    synchronized (lock) {
      ...
      switch (operator) {
        ...
        default:
          for (Map.Entry<String, HashMap<TriggerKey, TriggerWrapper>> entry :
              triggersByGroup.entrySet()) {
            if (operator.evaluate(entry.getKey(), compareToValue) && entry.getValue() != null) {
              if (outList == null) {
                outList = new HashSet<>();
              }
   ...
   *
   * <p>The JobStore should "remember" the groups paused, and impose the pause on any new triggers
   * that are added to one of these groups while the group is paused.
   */
  ...
  /**
   * Pause all of the <code>{@link org.quartz.JobDetail}s</code> in the given group - by pausing all
   * of their <code>Trigger</code>s.
   *
  ...
  public Collection<String> resumeJobs(GroupMatcher<JobKey> matcher) {
    ...
    synchronized (lock) {
      ...
      for (String pausedJobGroup : pausedJobGroups) {
        if (matcher
            .getCompareWithOperator()
            .evaluate(pausedJobGroup, matcher.getCompareToValue())) {
  ...
  public List<OperableTrigger> acquireNextTriggers(
    ...
    synchronized (lock) {
      ...
      if (timeTriggers.isEmpty()) return result;
      ...
      while (true) {
        ...
        // If trigger's job is set as @DisallowConcurrentExecution, and it has already been added to
        // result, then
        ...
        if (result.isEmpty()) {
          batchEnd =
              Math.max(tw.trigger.getNextFireTime().toEpochMilli(), System.currentTimeMillis())
                  + timeWindow;
        }
        result.add(trig);
      ...
      // If we did excluded triggers to prevent ACQUIRE state due to DisallowConcurrentExecution, we
      // need to add them back to store.
      if (!excludedTriggers.isEmpty()) timeTriggers.addAll(excludedTriggers);
      return result;
    }
  }
