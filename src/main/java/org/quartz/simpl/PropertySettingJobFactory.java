...
/**
 * A JobFactory that instantiates the Job instance (using the default no-arg constructor, or more
 * specifically: <code>class.newInstance()</code>), and then attempts to set all values from the
 * <code>SchedulerContext</code> and the <code>JobExecutionContext</code>'s merged <code>JobDataMap
 * </code> onto bean properties of the <code>Job</code>.
 *
 * <p>Set the warnIfPropertyNotFound property to true if you'd like noisy logging in the case of
 * values in the JobDataMap not mapping to properties on your Job class. This may be useful for
 * troubleshooting typos of property names, etc. but very noisy if you regularly (and purposely)
 * have extra things in your JobDataMap.
 *
 * <p>Also of possible interest is the throwIfPropertyNotFound property which will throw exceptions
 * on unmatched JobDataMap keys.
 *
