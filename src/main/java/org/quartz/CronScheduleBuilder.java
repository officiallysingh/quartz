...
public class CronScheduleBuilder extends ScheduleBuilder<CronTrigger> {
  ...
  /**
   * Create a CronScheduleBuilder with the given cron-expression string - which is presumed to be a
   ...
   *
   * @param daysOfWeek the day of the week to fire
   * @param hour the hour of day to fire
   * @param minute the minute of the given hour to fire
   * @return the new CronScheduleBuilder
   * @see CronExpression
  ...
  public static CronScheduleBuilder atHourAndMinuteOnGivenDaysOfWeek(
    ...
    StringBuilder cronExpression =
        new StringBuilder(String.format("0 %d %d ? * %d", minute, hour, daysOfWeek[0]));

    for (int i = 1; i < daysOfWeek.length; i++) cronExpression.append(",").append(daysOfWeek[i]);
