...
public class JobDetailTest {
  ...
  void testAnnotationDetection() {
    ...
    assertTrue(
        jobDetail.isPersistJobDataAfterExecution(), "Expecting SomePersistentJob to be persistent");
    assertFalse(
        jobDetail.isConcurrentExecutionDisallowed(),
        "Expecting SomePersistentJob to not disallow concurrent execution");

    jobDetail.setJobClass(SomeNonConcurrentJob.class);
    assertFalse(
        jobDetail.isPersistJobDataAfterExecution(),
        "Expecting SomeNonConcurrentJob to not be persistent");
    assertTrue(
        jobDetail.isConcurrentExecutionDisallowed(),
        "Expecting SomeNonConcurrentJob to disallow concurrent execution");

    jobDetail.setJobClass(SomeNonConcurrentPersistentJob.class);
    assertTrue(
        jobDetail.isPersistJobDataAfterExecution(),
        "Expecting SomeNonConcurrentPersistentJob to be persistent");
    assertTrue(
        jobDetail.isConcurrentExecutionDisallowed(),
    ...
    assertTrue(
        jobDetail.isPersistJobDataAfterExecution(),
        "Expecting SomeExtendedPersistentJob to be persistent");
    assertFalse(
        jobDetail.isConcurrentExecutionDisallowed(),
        "Expecting SomeExtendedPersistentJob to not disallow concurrent execution");

    jobDetail.setJobClass(SomeExtendedNonConcurrentJob.class);
    assertFalse(
        jobDetail.isPersistJobDataAfterExecution(),
        "Expecting SomeExtendedNonConcurrentJob to not be persistent");
    assertTrue(
        jobDetail.isConcurrentExecutionDisallowed(),
        "Expecting SomeExtendedNonConcurrentJob to disallow concurrent execution");

    jobDetail.setJobClass(SomeExtendedNonConcurrentPersistentJob.class);
    assertTrue(
        jobDetail.isPersistJobDataAfterExecution(),
        "Expecting SomeExtendedNonConcurrentPersistentJob to be persistent");
    assertTrue(
        jobDetail.isConcurrentExecutionDisallowed(),
