...
public class DailyTimeIntervalScheduleBuilderTest {
  ...
  void testScheduleActualTrigger() throws Exception {
    ...
    scheduler.scheduleJob(
        job, trigger); // We are not verify anything other than just run through the scheduler.
    scheduler.shutdown();
  }
  ...
  void testScheduleInMiddleOfDailyInterval() throws Exception {
    ...
    System.out.println(
        "testScheduleInMiddleOfDailyInterval: computed first fire time = "
            + trigger.getNextFireTime());

    assertTrue(
        trigger.getNextFireTime().isAfter(currTime.toInstant()),
        "First fire time is not after now!");

    Instant startTime = DateBuilder.todayAt(2, 15, 0);
  ...
  void testHourlyTrigger() {
    ...
    List<Instant> fireTimes = TriggerUtils.computeFireTimes((OperableTrigger) trigger, null, 48);
  ...
  void testMinutelyTriggerWithTimeOfDay() {
    ...
    assertTrue(Instant.now().toEpochMilli() >= trigger.getStartTime().toEpochMilli());
  ...
  void testSecondlyTriggerWithStartAndEndTime() {
    Instant startTime = DateBuilder.dateOf(0, 0, 0, 1, 1, 2011);
    Instant endTime = DateBuilder.dateOf(0, 0, 0, 2, 1, 2011);
    DailyTimeIntervalTrigger trigger =
        newTrigger()
    ...
    assertEquals(startTime.toEpochMilli(), trigger.getStartTime().toEpochMilli());
    assertEquals(endTime.toEpochMilli(), trigger.getEndTime().toEpochMilli());
  ...
  void testRepeatCountTrigger() {
    DailyTimeIntervalTrigger trigger =
        newTrigger()
            .withIdentity("test")
            .withSchedule(dailyTimeIntervalSchedule().withIntervalInHours(1).withRepeatCount(9))
            .build();
    assertEquals("test", trigger.getKey().getName());
    assertEquals("DEFAULT", trigger.getKey().getGroup());
    assertEquals(IntervalUnit.HOUR, trigger.getRepeatIntervalUnit());
  ...
  void testEndingAtAfterCountOf0() {
    ...
    try {
      ...
      newTrigger()
          .withIdentity("test")
          .withSchedule(
              dailyTimeIntervalSchedule().withIntervalInMinutes(15).endingDailyAfterCount(1))
          .startAt(startTime)
          .build();
